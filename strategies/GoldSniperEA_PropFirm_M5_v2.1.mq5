//+------------------------------------------------------------------+
//|                       GoldSniperEA_PropFirm_M5_v2.1.mq5          |
//|                    M5 OPTIMIZED PropFirm Edition                  |
//|                    Quality > Quantity - 60%+ Win Rate             |
//+------------------------------------------------------------------+
#property copyright "Copyright 2024"
#property link      ""
#property version   "2.1 M5 Quality Optimized"
#property strict
...
// Custom strategy files - UPDATED PATHS
#include "GoldTraderEA_Source\\CandlePatterns.mqh"
#include "GoldTraderEA_Source\\ChartPatterns.mqh"
#include "GoldTraderEA_Source\\PriceAction.mqh"
#include "GoldTraderEA_Source\\ElliottWaves.mqh"
#include "GoldTraderEA_Source\\Indicators.mqh"
#include "GoldTraderEA_Source\\Divergence.mqh"
#include "GoldTraderEA_Source\\HarmonicPatterns.mqh"
#include "GoldTraderEA_Source\\VolumeAnalysis.mqh"
#include "GoldTraderEA_Source\\WolfeWaves.mqh"
#include "GoldTraderEA_Source\\MultiTimeframe.mqh"
#include "GoldTraderEA_Source\\TimeAnalysis.mqh"
#include "GoldTraderEA_Source\\PivotPoints.mqh"
#include "GoldTraderEA_Source\\SupportResistance.mqh"
#include "GoldTraderEA_Source\\MACrossover.mqh"
#include "GoldTraderEA_Source\\TrendPatterns.mqh"

// PropFirm risk management functions
#include "PropFirm_Functions.mqh"

// Input values for settings
input string   Symbol_Name = "XAUUSD";          // Trading symbol
input ENUM_TIMEFRAMES Timeframe = PERIOD_M5;   // Timeframe [M5 AGGRESSIVE]
...
input double   Max_Lot_Size = 0.5;              // Maximum lot size [INCREASED]
input double   Max_Position_Volume = 2.0;       // Maximum volume of open positions [INCREASED]
input int      Max_Positions = 2;               // Maximum number of open positions [M5 v2.1: 2 safer]
input int      Max_Simultaneous_Trades = 2;     // Maximum number of simultaneous trades [M5 v2.1: 2]
input int      Min_Confirmations = 3;           // Minimum confirmations [M5 v2.1: 3 for quality]

// Declaration of input variables for general parameters
input string              General_Settings = "---- General Settings ----"; // Main parameters
input int                 Magic_Number = 123457;                  // Robot identification number [M5]
input bool                Require_MainTrend_Alignment = false;    // Align trades with the main trend [DISABLED for M5]

// Strategy activation - SNIPER MODE
input bool     Use_CandlePatterns = true;       // Use candle patterns
input bool     Use_ChartPatterns = true;        // Use chart patterns
input bool     Use_PriceAction = true;          // Use price action [CORE]
input bool     Use_ElliottWaves = false;        // Use Elliott waves [DISABLED - Too subjective]
input bool     Use_Indicators = true;           // Use indicators
input bool     Use_Divergence = true;           // Use divergences
input bool     Use_SupportResistance = true;    // Use support and resistance levels [CORE]
input bool     Use_HarmonicPatterns = false;    // Use harmonic patterns [DISABLED - Repainting]
...
input bool     Use_WolfeWaves = false;          // Use Wolfe waves [DISABLED]
...
input int      max_trades_per_candle = 3;      // Maximum number of trades allowed per candle [M5: 3]
...
// Weights of strategies [SNIPER WEIGHTING]
input int      CandlePatterns_Weight = 2;      // Weight of candle patterns [Booster]
input int      ChartPatterns_Weight = 3;       // Weight of chart patterns [Booster]
input int      PriceAction_Weight = 4;         // Weight of price action [CORE - High Weight]
input int      ElliottWaves_Weight = 0;        // Weight of Elliott waves [Disabled]
input int      Indicators_Weight = 1;          // Weight of indicators
input int      Divergence_Weight = 2;          // Weight of divergences
input int      SupportResistance_Weight = 5;   // Weight of support and resistance levels [CORE - Max Weight]
input int      HarmonicPatterns_Weight = 0;    // Weight of harmonic patterns [Disabled]
input int      MACrossover_Weight = 1;         // Weight of moving average crossovers
...
input int      WolfeWaves_Weight = 0;          // Weight of Wolfe waves [Disabled]
...
input bool     Trade_London_Session = true;     // Trade in London session [MAIN]
input int      London_Session_Start = 7;        // London session start time [M5: 07:00 EXTENDED]
input int      London_Session_End = 17;         // London session end time [M5: 17:00 EXTENDED]
input bool     Trade_NewYork_Session = true;    // Trade in New York session [M5: ENABLED]
input int      NewYork_Session_Start = 13;      // New York session start time (GMT)
input int      NewYork_Session_End = 21;        // New York session end time (GMT) [FULL SESSION]
input bool     Trade_Tokyo_Session = false;     // Trade in Tokyo session [DISABLED - Too volatile]
input int      Tokyo_Session_Start = 0;         // Tokyo session start time (GMT)
input int      Tokyo_Session_End = 6;           // Tokyo session end time (GMT)
input bool     Trade_Sydney_Session = false;    // Trade in Sydney session [DISABLED]
...
//| 🏦 PROPFIRM SETTINGS                                              |
//+------------------------------------------------------------------+
input string   PropFirm_Section = "===== 🏦 PROPFIRM MODE ====="; 
input bool     Use_PropFirm_Mode = true;                // Enable PropFirm strict rules
input string   PropFirm_Type = "FTMO";                  // Type: FTMO, MFF, E8, GENERIC
input double   Account_Size = 100000;                   // Account size for calculations

input string   Risk_Limits_Section = "===== 📉 RISK LIMITS =====";
input double   Max_Daily_Loss_Pct = 4.0;                // Max daily loss % (FTMO: 5%, safer: 4%)
input double   Max_Total_DD_Pct = 8.0;                  // Max total drawdown % (FTMO: 10%, safer: 8%)
input double   Daily_Profit_Target_Pct = 3.0;           // Daily profit target % [M5: 3%]
input double   Max_Daily_Profit_Pct = 45.0;             // Max daily profit as % of total (consistency rule)

input string   News_Section = "===== 📰 NEWS FILTER =====";
input bool     Avoid_News_Events = true;                // Avoid trading during major news
input int      News_Buffer_Minutes = 10;                // Minutes before/after news [M5: 10min]

input string   Weekend_Section = "===== 🔒 WEEKEND MANAGEMENT =====";
input bool     Close_Before_Weekend = true;             // Close all positions Friday evening
input int      Friday_Close_Hour = 20;                  // Hour to close (server time)

input string   Position_Section = "===== 💼 POSITION LIMITS =====";
input double   PropFirm_Risk_Per_Trade = 0.8;           // Max risk per trade % [M5 v2.1: 0.8% safer]
input int      PropFirm_Max_Positions = 2;              // Max simultaneous positions [M5 v2.1: 2]

// PropFirm global variables
double g_equity_start_day = 0;                          // Equity at day start
double g_balance_peak = 0;                              // Peak balance
datetime g_last_day_reset = 0;                          // Last daily reset time
bool g_trading_disabled_today = false;                  // Trading disabled flag
bool g_trading_disabled_global = false;                 // Permanent disable (DD exceeded)
int g_trading_days_count = 0;                           // Count of days with trades
double g_total_profit = 0;                              // Total profit since start
double g_today_profit = 0;                              // Today's profit
int g_trades_today = 0;                                 // Trades count today
...
int OnInit()
{
   ...
   // Initialize PropFirm risk management system
   PropFirm_Initialize();
...
void OnTick()
{
    ...
    if(Use_PropFirm_Mode) {
        // Check daily reset
        PropFirm_CheckDailyReset();
        
        // Check risk limits (daily loss, total DD, profit target)
        if(!PropFirm_CheckRiskLimits()) {
            return; // Trading disabled by PropFirm rules
        }
        
        // Check news filter
        if(PropFirm_IsNewsTime()) {
            return; // Trading paused during major news
        }
        
        // Check weekend close
        PropFirm_CheckWeekendClose();
        
        // Display dashboard (only in live mode)
